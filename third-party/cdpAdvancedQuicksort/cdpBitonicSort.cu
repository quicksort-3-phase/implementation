...
////////////////////////////////////////////////////////////////////////////////
template <typename T>
static __device__ __forceinline__ void bitonicsort_kernel(T *indata, T *outdata, unsigned int offset, unsigned int len, cg::thread_block cta) {
    __shared__ T sortbuf[1024];  // Max of 1024 elements - TODO: make this dynamic

    // First copy data into shared memory.
    unsigned int inside = (threadIdx.x < len);
    sortbuf[threadIdx.x] = inside ? indata[threadIdx.x + offset] : std::numeric_limits<T>::max();
    cg::sync(cta);
    ...
    for (unsigned int k = 2; k <= blockDim.x; k *= 2)  // Butterfly stride increments in powers of 2
    {
        for (unsigned int j = k >> 1; j > 0; j >>= 1)  // Strides also in powers of to, up to <k
        {
            ...
            // Both variables changed to T, now sorted correctly, but some elements are set to -1.
            T my_elem = sortbuf[threadIdx.x];
            T swap_elem = sortbuf[swap_idx];
...
////////////////////////////////////////////////////////////////////////////////
template <typename T>
static __device__ __forceinline__ void big_bitonicsort_kernel(T *indata, T *outdata, T *backbuf, unsigned int offset, unsigned int len, cg::thread_block cta) {
    ...
    for (unsigned int i = len; i < len2; i += blockDim.x) {
        ...
        if (index < len2) {
            ...
            if (index < len)
                indata[index + offset] = std::numeric_limits<T>::max();
            else
                backbuf[index + offset - len] = std::numeric_limits<T>::max();
        }
    }
...
template <typename T>
__global__ void bitonicsort(T *indata, T *outdata, unsigned int offset, unsigned int len) {
...
template <typename T>
__global__ void big_bitonicsort(T *indata, T *outdata, T *backbuf, unsigned int offset, unsigned int len) {
    // Handle to thread block group
    cg::thread_block cta = cg::this_thread_block();
