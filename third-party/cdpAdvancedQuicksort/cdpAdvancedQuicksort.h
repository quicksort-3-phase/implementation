#ifndef ADVANCED_QUICKSORT_H
#define ADVANCED_QUICKSORT_H
...
////////////////////////////////////////////////////////////////////////////////
template <typename T>
__global__ void qsort_warp(T *indata, T *outdata, unsigned int offset, unsigned int len, qsortAtomicData *atomicData, qsortRingbuf *atomicDataStack,
    ...
    T pivot = indata[offset + len / 2];
    T data = indata[offset + thread_id];
    ...
    if (lane_id == 0) {
        if (lt_count > 0) lt_offset = atomicAdd((unsigned int *)&atomicData->lt_offset, lt_count);

        if (gt_count > 0) gt_offset = len - (atomicAdd((unsigned int *)&atomicData->gt_offset, gt_count) + gt_count);
    }
    ...
    if (lane_id == 0) {
        ...
        if (atomicAdd((unsigned int *)&atomicData->sorted_count, mycount) + mycount == len) {
            ...
            if (lt_len == 0) {
                if (source_is_indata) cudaMemcpyAsync(indata + offset, outdata + offset, gt_len * sizeof(unsigned), cudaMemcpyDeviceToDevice, lstream);
            ...
            if (lt_len > BITONICSORT_LEN) {
                ...
                if (depth >= QSORT_MAXDEPTH) {
                    ...
                    if ((atomicData = ringbufAlloc<qsortAtomicData>(atomicDataStack)) == NULL)
                        ...
                        qsort_warp<<<numblocks, QSORT_BLOCKSIZE, 0, lstream>>>(outdata, indata, offset, lt_len, atomicData, atomicDataStack, !source_is_indata,
                                                                               depth + 1);
                    }
                }
            } else if (lt_len > 1) {
            ...
            if (cudaPeekAtLastError() != cudaSuccess) printf("Left-side launch fail: %s\n", cudaGetErrorString(cudaGetLastError()));
            ...
            if (gt_len > BITONICSORT_LEN) {
                ...
                if (depth >= QSORT_MAXDEPTH)
                    ...
                    if ((atomicData = ringbufAlloc<qsortAtomicData>(atomicDataStack)) == NULL)
                        ...
                        qsort_warp<<<numblocks, QSORT_BLOCKSIZE, 0, rstream>>>(outdata, indata, offset + lt_len, gt_len, atomicData, atomicDataStack, !source_is_indata,
                                                                               depth + 1);
                    }
                }
            } else if (gt_len > 1) {
            ...
            if (cudaPeekAtLastError() != cudaSuccess) printf("Right-side launch fail: %s\n", cudaGetErrorString(cudaGetLastError()));
        }
    }
}
...
////////////////////////////////////////////////////////////////////////////////
template <typename T>
float run_quicksort_cdp(T *gpudata, T *scratchdata, unsigned int count, cudaStream_t stream) {
...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
// Added time tracking variables, out commented seed & switched from unsigned to uint64_t
template <typename T>
int run_qsort(uint64_t size, /*int seed*/ T *data, int debug, int loop, int verbose, double &startTime, double &endTime) {
    /*if (seed > 0)
        srand(seed);*/

    // Create and set up our test
    T *gpudata, *scratchdata;
    checkCudaErrors(cudaMalloc((void **)&gpudata, size * sizeof(T)));
    checkCudaErrors(cudaMalloc((void **)&scratchdata, size * sizeof(T)));
    checkCudaErrors(cudaMemcpy(gpudata, data, size * sizeof(T), cudaMemcpyHostToDevice));
    ...
    checkCudaErrors(cudaMemcpy(data, gpudata, size * sizeof(T), cudaMemcpyDeviceToHost));
    ...
    if (verbose) {
        ...
        printf("Sorted %lu elems in %.3f ms (%.3f Melems/sec)\n", size, elapse, (float)size / (elapse * 1000.0f));
    }
    fflush(stdout);
